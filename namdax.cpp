...
namespace ExpertMultimediaBase {
	...
	*/
		...
		try {
			...
			if (iGameState==GAMESTATE_INIT) {
				...
				while (!bStart) { //mini polling loop THREE OF FOUR! (show intro text)
					...
					gbIntroTextBack.DrawToLargerWithoutCropElseCancel(gbScreen, SCREEN_OFFSET_X, SCREEN_OFFSET_Y, DrawModeCopyAlpha);//TargaToTarga32(gbIntroTextBack, gbScreen, SCREEN_OFFSET_X+(SCREEN_WIDTH/2-gbIntroTextBack->width/2),SCREEN_OFFSET_Y);
	...
	/*inline*/inline void SleepWrapper(int iTicks) {
		SDL_Delay(iTicks);
	}
...
int main(int iArgs, char *lpsArg[]) { //int main(int iArgs, char** lpsArg) {
	...
	while (!bDone) {
		...
		if (bFirstRunOfMainEventLoop) Console.Error.WriteLine("FirstRun: Checking whether user exited early...");
